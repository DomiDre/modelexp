...
class SphereCSStackedSpacer(ReflectometryModel):
  '''
  ...
  def initParameters(self):
    ...
    self.params.add("roughnessSlope", 0, min = 0.0, max = 1.0, vary = True)
  ...
  def calcModel(self):
    if (self.z is not None):
      sphere_shifts = [self.params["layerDistance"].value] * int(self.params['nPeriods'].value)
      packing_densities = [self.params["packingDensity"].value] * int(self.params['nPeriods'].value)
