...
class Gui:
  def __init__(self):
    ...
    mainWindow.setWindowTitle("model.py GUI")
    mainWindow.show()
    app.exec_()
...
class GuiMainWindow(qt5w.QMainWindow):
  def __init__(self):
    ...
    # main widgets
    self.mainWidget = qt5w.QWidget(self) # widget that contains everything
    self.plotWidget = qt5w.QWidget(self) # widget to include plot window
    ...
    self.layout = qt5w.QGridLayout(self.mainWidget)
    self.layout.addWidget(self.plotWidget, 0, 0) # upper left
    ...
    self.mainWidget.setFocus() # set focus onto the main widget
